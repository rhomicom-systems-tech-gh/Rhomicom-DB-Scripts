...
CREATE OR REPLACE FUNCTION pay.get_tk_loan_req_cur_bals (p_request_id bigint , p_replace_str character varying)
	RETURNS numeric
	LANGUAGE 'plpgsql'
	COST 100 VOLATILE PARALLEL UNSAFE
	AS $BODY$
...
CREATE OR REPLACE FUNCTION pay.get_trn_pymnt_dte_intvl (p_person_id bigint , p_pay_itm_id bigint)
	RETURNS numeric
	LANGUAGE 'plpgsql'
	COST 100 VOLATILE PARALLEL UNSAFE
	AS $BODY$
...
CREATE OR REPLACE FUNCTION pay.get_payitm_expctd_amnt (p_itm_id integer , p_prsn_id bigint , p_org_id integer , p_datestr character varying)
	RETURNS numeric
	LANGUAGE 'plpgsql'
	COST 100 VOLATILE PARALLEL UNSAFE
	AS $BODY$
...
BEGIN
	...
	END IF;
	SELECT
		pssbl_value_sql INTO val_sql
	FROM
		org.org_pay_items_values
	WHERE
		pssbl_value_id = p_itmValID;
	IF COALESCE(val_sql , '') = '' THEN
		SELECT
			pssbl_amount INTO valamnt FROM org.org_pay_items_values
		WHERE
			pssbl_value_id = p_itmValID;
	ELSE
